...
仓库现在包含 **SRT Whiteboard Studio**：一个只在本机 `127.0.0.1` 运行的浏览器工作台。它可以批量导入图片或文件夹并按编号排序，选择手绘动画或保留原图的静态视频模式，读取脚本/TXT、voice 与背景音乐，按 voice 总时长自动分配场景，调整单幕时长、绘制速度、墨色、音量、字幕和频道名，并用平滑转场导出 9:16 或 16:9 MP4。失败的场景可单独重渲染，再与其余场景合并。
