...
仓库现在包含 **SRT Whiteboard Studio**：一个只在本机 `127.0.0.1` 运行的浏览器工作台。它可以批量导入图片或文件夹并按编号排序，选择手绘动画或保留原图的静态视频模式，读取脚本/TXT、voice 与背景音乐，按 voice 总时长自动分配场景，调整单幕时长、绘制速度、墨色、音量、字幕和频道名，并用平滑转场导出 9:16 或 16:9 MP4。导入 `source-script-vi.txt` 后，合并视频时还会同步生成与 voice 时间线对齐的越南语 YouTube SRT。失败的场景可单独重渲染，再与其余场景合并。

Windows 用户双击 `START_WHITEBOARD_APP.bat` 即可准备隔离环境并打开应用。越南语使用说明见 [HUONG-DAN-APP.md](HUONG-DAN-APP.md)，Windows 环境说明见 [README.windows.md](README.windows.md)。所有用户素材与成片保存在 Git 忽略的 `workspace/` 目录。
