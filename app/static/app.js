...
function renderStudio(){
  ...
  showVolume();
  $('#analysisInfo').textContent=a.voice_duration?`Đã căn theo voice ${a.voice_duration.toFixed(2)} giây.`:`Chưa có voice; đang dùng thời lượng ước tính ${Number(a.total_duration||0).toFixed(2)} giây.`;
  ...
  $$('.preview-scene').forEach(button=>button.onclick=()=>togglePreview(Number(button.dataset.index)));
...
function sceneCard(scene,id){
  return `<article class="scene-card" data-index="${scene.index}">
    <div class="scene-media" id="media-${scene.index}"><img loading="lazy" src="/api/projects/${id}/images/${scene.index}" alt="Cảnh ${scene.index}"><span class="scene-no">CẢNH ${String(scene.index).padStart(2,'0')}</span><span class="scene-status">${scene.rendered?'ĐÃ DỰNG':'SẴN SÀNG'}</span></div>
    <div class="scene-body"><div class="scene-name"><strong>${esc(scene.image)}</strong><small title="${esc(scene.source_name)}">${esc(scene.source_name)}</small></div>
      ...
      <div class="scene-actions"><button class="secondary render-scene" data-index="${scene.index}">${scene.rendered?'↻ Dựng lại cảnh':'▶ Dựng cảnh'}</button>${scene.rendered?`<button class="ghost preview-scene" data-index="${scene.index}">Xem video</button>`:''}</div>
    </div></article>`;
}
...
function togglePreview(index){const media=$(`#media-${index}`);if(media.querySelector('video')){media.innerHTML=`<img src="/api/projects/${state.project.id}/images/${index}" alt="Cảnh ${index}"><span class="scene-no">CẢNH ${String(index).padStart(2,'0')}</span><span class="scene-status">ĐÃ DỰNG</span>`}else{media.innerHTML=`<video controls autoplay src="/api/projects/${state.project.id}/scenes/${index}/video?t=${Date.now()}"></video><span class="scene-no">CẢNH ${String(index).padStart(2,'0')}</span>`}}
...
$('#voiceVolume').oninput=$('#musicVolume').oninput=showVolume;$('#saveProject').onclick=()=>save();$('#reanalyze').onclick=reanalyze;$('#renderAll').onclick=startAll;$('#mergeOnly').onclick=mergeOnly;$('#closeProgress').onclick=progressClose;
